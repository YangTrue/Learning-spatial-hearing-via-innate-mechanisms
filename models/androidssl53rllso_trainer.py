...
class androidssl53rllsotrainer(BaseTrainer, androidssl52baseRunner):
    """
    53rllso
    ...
    def optimize_parameters(self, total_iters):
        ...
        with AccumulatedOptimizeNetContextManager(self.model, self.model.model_names, self, total_iters, self.opt.opt_cycle):
            ...
            for i in range(self.opt.max_rollout_steps):                
                ...
                env.transit_wo_reward(action[0,0], sw_render=False) # avoid wasted rendering 
                ...
                if angle > 90 or angle < -90:
                    ...
                    midline_score = self.model.netmidline(torch.from_numpy(left_lso_array_sample).float(), torch.from_numpy(right_lso_array_sample).float()) # return a vector of 32 neurons, right 0, left 1
