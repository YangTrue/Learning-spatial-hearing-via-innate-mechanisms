...
class androidssl52baseRunner(BaseRunner):
    """
    52baseRunner

    change the action method
    take in the KEMAR spectrum as input rather than the simple scalar
    # -------
    ...
    def action(self, input):
        """
        input should be a single vector e.g. [32]
        ...
        y_pred = self.model.netandroid(input)
        
        mu = torch.sigmoid(y_pred) # location, real angle is [0.25, 0.75] if scaling factor is 1. 

        # should not do the scaling here, the sigmoid is wrongly initialized otherwise, it starts from 0.5
        # scale the mu so that learning is easier
        # otherwise, the boundary part is harder to reach
        # mu = torch.sigmoid(y_pred)*2 # true output should be [0.25, 0.75]
        # mu = torch.clip(mu, 0, 1.0)  

        # NOTE: remove the leared std, so that we simplify the code        
        # # tau = nnf.softplus(y_pred[0, 1])+1e-9
        # # std = torch.sqrt(1.0/(tau+1e-9))/10.0

        # # bounded to (0, 0.1)
        # std = torch.abs(torch.tanh(y_pred[1])) / 100 
        ...
        scale_factor=2.0
        std=torch.tensor(self.opt.policy_sigma/scale_factor).to(mu.device)
